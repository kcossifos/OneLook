...
//current URL of the Page. elpotro_update.php redirects back to this URL
...
if(isset($_SESSION["elpotro_items"]) && count($_SESSION["elpotro_items"])>0)
{
	...
	echo '<td colspan="4">';
	echo '<button type="submit">Update</button><a href="elpotro_cart.php" class="button">Checkout</a>';
	echo '</td>';
	echo '</tbody>';
	echo '</table>';
...
<!-- items List Start -->
<?php
$data = $mysqli->query("SELECT item_id, item_name, item_price, menu_id, category_id, descrip, section_id, code FROM OneLook_items_continued WHERE menu_id = '13'");
if($data){
