...
//current URL of the Page. moghul_update.php redirects back to this URL
...
if(isset($_SESSION["moghul_items"]) && count($_SESSION["moghul_items"])>0)
{
	...
	echo '<td colspan="4">';
	echo '<button type="submit">Update</button><a href="moghul_cart.php" class="button">Checkout</a>';
	echo '</td>';
	echo '</tbody>';
	echo '</table>';
...
<!-- items List Start -->
<?php
$data = $mysqli->query("SELECT item_id, item_name, item_price, menu_id, category_id, descrip, section_id, code FROM OneLook_items_continued WHERE menu_id = '10'");
if($data){
