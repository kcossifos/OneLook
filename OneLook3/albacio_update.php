...
if(isset($_POST["type"]) && $_POST["type"]=='add' && $_POST["item_qty"]>0)
{
	...
	while($statement->fetch()){
        ...
        if(isset($_SESSION["albacio_items"])){
            if(isset($_SESSION["albacio_items"][$new_menuitem['code']]))
            {
                unset($_SESSION["albacio_items"][$new_menuitem['code']]);
            }
        }
        $_SESSION["albacio_items"][$new_menuitem['code']] = $new_menuitem;
    }
}
...
if(isset($_POST["item_qty"]) || isset($_POST["remove_code"]))
{
	if(isset($_POST["item_qty"]) && is_array($_POST["item_qty"])){
		foreach($_POST["item_qty"] as $key => $value){
			if(is_numeric($value)){
				$_SESSION["albacio_items"][$key]["item_qty"] = $value;
			}
		}
	}
	if(isset($_POST["remove_code"]) && is_array($_POST["remove_code"])){
		foreach($_POST["remove_code"] as $key){
			unset($_SESSION["albacio_items"][$key]);
		}
	}
}
