...
//current URL of the Page. giovanni_update.php redirects back to this URL
...
if(isset($_SESSION["giovanni_items"]) && count($_SESSION["giovanni_items"])>0)
{
	...
	echo '<td colspan="4">';
	echo '<button type="submit">Update</button><a href="giovanni_cart.php" class="button">Checkout</a>';
	echo '</td>';
	echo '</tbody>';
	echo '</table>';
...
<!-- items List Start -->
<?php
$data = $mysqli->query("SELECT item_id, item_name, item_price, menu_id, category_id, descrip, section_id, code FROM OneLook_items WHERE menu_id = '1'");
if($data){
