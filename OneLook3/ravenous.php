...
//current URL of the Page. ravenous_update.php redirects back to this URL
...
if(isset($_SESSION["ravenous_items"]) && count($_SESSION["ravenous_items"])>0)
{
	...
	echo '<td colspan="4">';
	echo '<button type="submit">Update</button><a href="ravenous_cart.php" class="button">Checkout</a>';
	echo '</td>';
	echo '</tbody>';
	echo '</table>';
...
<!-- items List Start -->
<?php
$data = $mysqli->query("SELECT item_id, item_name, item_price, menu_id, category_id, descrip, section_id, code FROM OneLook_items_continued_2 WHERE menu_id = '16'");
if($data){
