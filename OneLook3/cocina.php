...
//current URL of the Page. cocina_update.php redirects back to this URL
...
<!-- View Cart Box Start -->
<?php
if(isset($_SESSION["cocina_items"]) && count($_SESSION["cocina_items"])>0)
{
	echo '<div class="shoppingcart" id="view-cart">';
	echo '<h3>Your Menu Items</h3>';
	echo '<form method="post" action="cocina_update.php">';
	...
	echo '<td colspan="4">';
	echo '<button type="submit">Update</button><a href="cocina_cart.php" class="button">Checkout</a>';
	echo '</td>';
	echo '</tbody>';
	echo '</table>';
...
<!-- items List Start -->
<?php
$data = $mysqli->query("SELECT item_id, item_name, item_price, menu_id, category_id, descrip, section_id, code FROM OneLook_items_continued WHERE menu_id = '11'");
if($data){
